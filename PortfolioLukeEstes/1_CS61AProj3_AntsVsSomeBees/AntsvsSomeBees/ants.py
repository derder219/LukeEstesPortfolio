    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                new_ant = insect
                if self.ant and new_ant.can_contain(new_ant):
                    self.ant.contain_ant(new_ant)
                elif self.ant and new_ant.can_contain(self.ant):
                    new_ant.ant.contain_ant(self.ant)
                    self.ant = new_ant
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        target = self.place
        while target != hive:
        	if random_or_none(target.bees) == None:
        		target = target.entrance
        		counter += 1
        	elif not(counter <= self.max_range and counter >= self.min_range):
        		target = target.entrance
        		counter += 1
        	else:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
        	...
        	if not(random_or_none(self.place.bees) == None):
